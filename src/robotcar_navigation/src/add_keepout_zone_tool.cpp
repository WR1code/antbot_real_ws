...
AddKeepoutZoneTool::AddKeepoutZoneTool()
{
  shortcut_key_ = 'k';
  setCursor(QCursor(Qt::CrossCursor));
  name_property_ = new rviz_common::properties::StringProperty(
    "Zone Name", "临时禁区", "保存禁区时使用的名称。", getPropertyContainer());
...
void AddKeepoutZoneTool::onInitialize()
{
  ...
  rectangle_preview_->setColor(0.95F, 0.12F, 0.12F, 0.34F);
  ellipse_preview_->setColor(0.95F, 0.12F, 0.12F, 0.34F);
  outline_preview_->setColor(1.0F, 0.16F, 0.08F, 0.98F);
...
void AddKeepoutZoneTool::activate()
{
  drawing_ = false;
  hidePreview();
  setStatus("选择形状后，在地图上按住左键从一个角拖到对角；松开即添加禁区。");
}

void AddKeepoutZoneTool::deactivate()
{
  drawing_ = false;
  hidePreview();
}

Ogre::Vector3 AddKeepoutZoneTool::constrainedEnd(const Ogre::Vector3 & raw_end) const
{
...
std::string AddKeepoutZoneTool::shapeName() const
{
...
void AddKeepoutZoneTool::updatePreview(const Ogre::Vector3 & raw_end)
{
  ...
  const Ogre::ColourValue color(1.0F, 0.16F, 0.08F, 0.98F);
  if (ellipse) {
  ...
  setStatus(
    QString("正在绘制：%1 × %2 m；松开左键完成。")
    .arg(width, 0, 'f', 2).arg(height, 0, 'f', 2));
}

void AddKeepoutZoneTool::hidePreview()
{
...
void AddKeepoutZoneTool::publishZone(const Ogre::Vector3 & raw_end)
{
  const Ogre::Vector3 end = constrainedEnd(raw_end);
  const float width = std::abs(end.x - drag_start_.x);
  const float height = std::abs(end.y - drag_start_.y);
  ...
  setStatus(
    QString("已添加禁区：%1 × %2 m；可继续拖动添加下一个。").arg(width, 0, 'f', 2)
    .arg(height, 0, 'f', 2));
}

int AddKeepoutZoneTool::processMouseEvent(rviz_common::ViewportMouseEvent & event)
{
...
void AddKeepoutZoneTool::updateTopic()
{
  if (node_) {
    publisher_ = node_->create_publisher<msg::KeepoutZone>(
...
PLUGINLIB_EXPORT_CLASS(
  robotcar_navigation::rviz_plugins::AddKeepoutZoneTool,
  rviz_common::Tool)
